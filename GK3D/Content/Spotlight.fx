...
VertexShaderOutput VertexShaderFunction(VertexShaderInput input)
{
    ...
    output.Color = saturate(input.Color + max( DiffuseColor * DiffuseIntensity * lightIntensity, 0));
...
{
    float3 light = normalize(LightPosition - input.OriginalPosition);
    ...
    return saturate(input.Color + max(AmbientColor * AmbientIntensity,0) + max(0,specular));
}
